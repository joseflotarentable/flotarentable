...
function FlotaPage({userId,perfil,updatePerfil}) {
  ...
  useEffect(()=>{
    ...
    if(perfil.empresa_id){sb.from("empresas").select("codigo").eq("id",perfil.empresa_id).single().then(({data})=>setCodigo(data?.codigo||""));}
  },[]);
    ...
    <div className="page fu">
      ...
      {tab==="flota"&&<>
        <div>
          ...
          {!loading&&tractoras.length===0&&<div className="empty" style={{padding:"1rem"}}><div className="ei"><Icon d={I.truck} size={18} color="var(--muted)"/></div><span style={{fontSize:"0.8rem"}}>Sin tractoras</span></div>}
        ...
        <div>
          ...
          {!loading&&semis.length===0&&<div className="empty" style={{padding:"1rem"}}><div className="ei"><Icon d={I.gear} size={18} color="var(--muted)"/></div><span style={{fontSize:"0.8rem"}}>Sin semirremolques</span></div>}
          <div style={{display:"flex",flexDirection:"column",gap:"0.5rem"}}>
      ...
      {tab==="ajustes"&&<>
        ...
        {codigo&&perfil.rol==="gerente"&&<div className="card">
          <div className="chd">Código para chóferes</div>
          <p style={{fontSize:"0.82rem",color:"var(--muted)",marginBottom:"0.75rem"}}>Comparte este código con tus chóferes:</p>
...
function GastosPage({userId,tractoras,semis,esGerente,accentIdx}) {
  ...
  const deleteGasto=async id=>{await sb.from("gastos").delete().eq("id",id);setGastos(gastos.filter(g=>g.id!==id));setToast("🗑️ Eliminado");};
...
function InicioPage({userId,tractoras,semis,perfil,esGerente,gastosTodos,viajesTodos,setViajesTodos,gastosFijos}) {
  ...
  const cktMedia=tractoras.reduce((s,t)=>{const km=parseFloat(t.km_mensuales)||0;if(!km)return s;const fijosT=gastosFijos.filter(g=>g.entidad_id===t.id);const total=fijosT.reduce((a,g)=>{const imp=parseFloat(g.importe)||0;return a+(g.periodo==="anual"?imp/12:imp);},0);return s+total/km;},0)/Math.max(tractoras.filter(t=>t.km_mensuales).length,1);
  ...
  return(
    <div className="page fu">
      ...
      {esGerente&&cktMedia>0&&<div className="hcard">
        ...
        <div className="hsub">Basado en gastos fijos configurados · {tractoras.length} tractora{tractoras.length!==1?"s":""}</div>
      </div>}
