...
function gastoProrrateadoEnMes(g, mes) {
...
// ── GASTOS PAGE ───────────────────────────────────────────────────────────────
function CustomConceptoRow({entidadId, saveFijo}) {
  ...
  return(<>
    ...
    {modal&&<div className="ov" onClick={()=>setModal(false)}>
        <div className="modal" onClick={e=>e.stopPropagation()}>
          ...
          {esGerente&&<div className="toggle-row"><span className="toggle-lbl">Precio incluye IVA</span><button className={`toggle ${modal.tiene_iva?"on":""}`} onClick={()=>setModal(f=>({...f,tiene_iva:!f.tiene_iva}))}/></div>}
...
function AjustesModal({userId,perfil,updatePerfil,onClose,onLogout}) {
  ...
  useEffect(()=>{
    if(perfil.empresa_id){
      sb.from("empresas").select("codigo").eq("id",perfil.empresa_id).single().then(({data})=>{if(data?.codigo)setCodigo(data.codigo);});
    } else {
      sb.from("empresas").select("id,codigo").eq("gerente_id",userId).single().then(async({data:emp})=>{
        if(emp){
        ...
        {perfil.rol==="gerente"&&<div className="card">
          ...
          {codigo
            ...
            :<button className="btn bp" onClick={async()=>{
              const{data:existing}=await sb.from("empresas").select("id,codigo").eq("gerente_id",userId).single();
              if(existing){await sb.from("perfiles").update({empresa_id:existing.id}).eq("id",userId);updatePerfil({empresa_id:existing.id});setCodigo(existing.codigo);return;}
              const cod=genCode();
              ...
              if(emp){await sb.from("perfiles").update({empresa_id:emp.id}).eq("id",userId);updatePerfil({empresa_id:emp.id});setCodigo(emp.codigo);}
...
export default function App() {
      ...
      if(session?.user){
        ...
        if(p?.rol==="chofer"&&p?.empresa_id){
          ...
          if(emp?.gerente_id){
            ...
            const{data:gp}=await sb.from("perfiles").select("trial_start").eq("id",emp.gerente_id).single();
        ...
        let vTodos=v||[];
        if(p?.rol!=="chofer"&&p?.empresa_id){
          const{data:gEmp}=await sb.from("gastos").select("*").neq("user_id",session.user.id);
          const{data:vEmp}=await sb.from("viajes").select("*").neq("user_id",session.user.id).order("fecha",{ascending:false});
          if(gEmp)gTodos=[...gTodos,...gEmp];
          if(vEmp)vTodos=[...vTodos,...vEmp];
        }
        setTractoras(t||[]);setSemis(s||[]);setGastosTodos(gTodos);setGastosFijos(gf||[]);setViajesTodos(vTodos);
      }
      setLoading(false);
    });
  },[]);
  ...
  return(
    ...
    <div className="app">
      ...
      {tab==="viajes"&&<ViajesPage key={`viajes-${tractorasActivas.length}-${semisActivas.length}`} userId={user.id} tractoras={tractorasActivas} semis={semisActivas} esGerente={esGerente} gastosTodos={gastosTodos}/>}
      {tab==="gastos"&&<GastosPage key={`gastos-${tractorasActivas.length}-${semisActivas.length}`} userId={user.id} tractoras={tractorasActivas} semis={semisActivas} esGerente={esGerente} accentIdx={perfil.accent_idx||0} gastosFijos={gastosFijos} setGastosFijos={setGastosFijos}/>}
