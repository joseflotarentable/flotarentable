    ...
    sb.auth.getSession().then(async({data:{session}})=>{
      if(session?.user){
        ...
        if(p?.rol==="chofer"&&p?.empresa_id){
          ...
          if(emp?.gerente_id)tractorUserId=emp.gerente_id;
        }
        const[{data:t},{data:s},{data:g},{data:gf}]=await Promise.all([
        ...
        setTractoras(t||[]);setSemis(s||[]);setGastosTodos(g||[]);setGastosFijos(gf||[]);
      }
      setLoading(false);
    });
  },[]);
