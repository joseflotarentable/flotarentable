...
const TIPOS_GASTO_VAR = ["Combustible","Peaje","Mantenimiento","Neumáticos","Avería","ITV","Lavado","Seguro","Otros"];
const CONCEPTOS_EMPRESA = ["Gestoría","Impuestos","Autónomo","Asesoría","Maquinaria","Seguro empresa","Otros"];
const CONCEPTOS_VEHICULO = ["Parking","Leasing / Renting","Nómina chófer","Autónomo","Seguro anual","ITV","Otros"];
...
async function geocode(q) {
  ...
  try {
    const r = await fetch(`https://nominatim.openstreetmap.org/search?q=${encodeURIComponent(q)}&format=json&limit=5&addressdetails=1`,{headers:{"Accept-Language":"es","User-Agent":"KmRentable/1.0"}});
    const data = await r.json();
    const res = data.map(x=>({label:x.display_name.split(",").slice(0,3).join(", "),lat:parseFloat(x.lat),lon:parseFloat(x.lon)}));
...
// Coste €/km completo: fijos vehículo + parte empresa + variables del mes + combustible estimado
function calcCosteKmCompleto(tractora, gastosFijos, gastosVar, viajes, kmMes) {
  const km = parseFloat(tractora.km_mensuales)||kmMes||1;
  // Fijos del vehículo
  ...
  // Fijos empresa repartidos entre tractoras (entidad_id="empresa")
  ...
  const numTractoras = Math.max(1, gastosFijos.filter(g=>g.entidad_id!=="empresa").map(g=>g.entidad_id).filter((v,i,a)=>a.indexOf(v)===i).length);
  const fijosEPorTractora = fijosE / numTractoras;
  // Variables del mes de esta tractora
  const varMes = gastosVar.filter(g=>g.vehicle_id===tractora.id&&g.mes===nowMes()).reduce((s,g)=>s+(parseFloat(g.importe)||0),0);
  // Combustible estimado por km
  ...
  // km reales del mes de los viajes
  const kmReales = viajes.filter(v=>v.truck_id===tractora.id&&v.fecha?.startsWith(nowMes())).reduce((s,v)=>s+(parseFloat(v.km)||0)+(parseFloat(v.km_vuelta)||0),0)||km;
  return (fijosV+fijosEPorTractora)/km + varMes/kmReales + combustibleKm;
}
  ...
  return(
    <div style={{position:"relative"}}>
      ...
      {open&&<div style={{position:"absolute",top:"100%",left:0,right:0,background:"var(--s1)",border:"1px solid var(--border2)",borderRadius:"var(--r2)",zIndex:200,maxHeight:180,overflowY:"auto",marginTop:4,boxShadow:"0 8px 24px #00000060"}}>
        {sugs.map((s,i)=><div key={i} style={{padding:"0.625rem 0.875rem",fontSize:"0.82rem",cursor:"pointer",borderBottom:"1px solid var(--border)",color:"var(--text)"}} onMouseDown={()=>{onChange(s.label.split(",")[0].trim());onSelect(s);setOpen(false);}}>{s.label}</div>)}
      </div>}
    </div>
  );
}
  ...
  if(mode==="welcome")return(
    <div className="auth-wrap fu">
      <div style={{position:"relative",padding:"3rem 1.75rem 2rem",textAlign:"center",overflow:"hidden",display:"flex",flexDirection:"column",alignItems:"center",gap:"1.25rem"}}>
        ...
        <div><div className="auth-wordmark">Km<br/>Rentable</div><p style={{position:"relative",zIndex:1,fontSize:"0.9rem",color:"var(--muted)",lineHeight:1.65,maxWidth:270}}>Tu negocio de transporte en el bolsillo.</p></div>
      </div>
    ...
    <div className="page fu">
      ...
      {tab==="flota"&&<>
          ...
          <div style={{display:"flex",flexDirection:"column",gap:"0.5rem"}}>
            {tractoras.map(t=>{const semi=semis.find(s=>s.id===t.semi_habitual_id);return(
              <div key={t.id} className="vcard" onClick={()=>setEditT(t)}>
                ...
                <div style={{flex:1}}><div style={{fontWeight:700,fontSize:"0.875rem"}}>{t.matricula||"Sin matrícula"}</div><div style={{fontSize:"0.7rem",color:"var(--muted)",marginTop:1}}>{t.subtipo||"Tractora"}</div>{t.apodo&&<div style={{fontSize:"0.7rem",color:"var(--a2)",marginTop:1,fontWeight:600}}>"{t.apodo}"</div>}{semi&&<div className="semi-tag" style={{marginTop:3}}><Icon d={I.link} size={10}/>{semi.matricula}{t.conjunto_fijo?" · fijo":""}</div>}</div>
        ...
        <div>
          ...
          {semis.length===0&&<div className="empty" style={{padding:"1rem"}}><div className="ei"><Icon d={I.gear} size={18} color="var(--muted)"/></div><span style={{fontSize:"0.8rem"}}>Sin semirremolques</span></div>}
          <div style={{display:"flex",flexDirection:"column",gap:"0.5rem"}}>
      ...
      {tab==="ajustes"&&<>
        <div className="card">
          ...
          <div style={{display:"flex",flexDirection:"column",gap:"0.75rem"}}>
            <div className="fld"><label className="lbl">Nombre / Empresa</label><InputGuardado valor={perfil.empresa||""} placeholder="Transportes García S.L." onGuardar={v=>saveAjustes({empresa:v})}/></div>
        ...
        {perfil.rol==="gerente"&&<div className="card">
          ...
          <p style={{fontSize:"0.82rem",color:"var(--muted)",marginBottom:"0.75rem"}}>Comparte este código con tus chóferes para que se registren:</p>
          {codigo
            ...
            :<button className="btn bp" onClick={async()=>{const cod=genCode();const{data:emp}=await sb.from("empresas").insert({nombre:perfil.empresa||perfil.nombre||"Mi empresa",codigo:cod,gerente_id:userId}).select().single();if(emp){await sb.from("perfiles").update({empresa_id:emp.id}).eq("id",userId);setCodigo(emp.codigo);}else{setCodigo(cod);}}}>Generar código</button>
          }
        </div>}
        <div className="card">
    ...
    <div className="page fu">
      ...
      <div className="card">
        ...
        <div className="g2" style={{marginTop:"0.75rem"}}>
          ...
          <div className="fld"><label className="lbl">Km/mes (para coste/km)</label><input className="inp" type="number" value={form.km_mensuales||""} placeholder="8000" onChange={e=>setForm({...form,km_mensuales:e.target.value})}/></div>
        </div>
      </div>
...
// ── GASTOS PAGE ───────────────────────────────────────────────────────────────
function GastosPage({userId,tractoras,semis,esGerente,accentIdx}) {
  ...
  const emptyForm={fecha:new Date().toISOString().slice(0,10),tipo:"Combustible",importe:"",litros:"",precio_litro:"",odometro:"",pais:"España",vehicle_id:tractoras[0]?.id||"",vehicle_tipo:"tractora",nota:"",mes:mesFiltro,ano:anoActual};

  const openNew=()=>{setEditGasto(null);setModal({...emptyForm});};
  const openEdit=g=>{setEditGasto(g);setModal({...g});};

  const saveGasto=async()=>{
    if(!modal.importe)return;
    if(editGasto?.id){
      await sb.from("gastos").update({...modal}).eq("id",editGasto.id);
      setGastos(gastos.map(g=>g.id===editGasto.id?{...g,...modal}:g));
      setToast("✅ Gasto actualizado");
    }else{
      const{data,error}=await sb.from("gastos").insert({...modal,user_id:userId,mes:mesFiltro,ano:anoActual}).select();
                ...
                {v.conceptos.map(concepto=>{
                  ...
                  return(
                    <div key={concepto} className="gfijo-row">
                      ...
                      <input className="gfijo-inp" type="number" placeholder="0" value={fijo?.importe||""} onChange={e=>saveFijo(v.id,concepto,e.target.value,esAnual?"anual":"mensual")}/>
      ...
      {modal&&<div className="ov" onClick={()=>setModal(false)}>
        <div className="modal" onClick={e=>e.stopPropagation()}>
          ...
          <div className="g2">
            ...
            <div className="fld"><label className="lbl">Tipo</label><select className="inp sel" value={modal.tipo} onChange={e=>setModal({...modal,tipo:e.target.value})}>{TIPOS_GASTO_VAR.map(t=><option key={t}>{t}</option>)}</select></div>
          </div>
          ...
          <div className="fld"><label className="lbl">Importe (€){modal.tipo==="Combustible"&&modal.litros?<span style={{color:"var(--green)",fontSize:"0.68rem"}}> · calculado auto</span>:""}</label><input className="inp" type="number" placeholder="0,00" value={modal.importe} onChange={e=>setModal({...modal,importe:e.target.value})}/></div>
          <div className="fld"><label className="lbl">Vehículo</label>
            <select className="inp sel" value={modal.vehicle_id} onChange={e=>{const todos=[...tractoras,...semis];const veh=todos.find(v=>v.id===e.target.value);setModal({...modal,vehicle_id:e.target.value,vehicle_tipo:tractoras.find(t=>t.id===e.target.value)?"tractora":"semi"});}}>
...
function ViajesPage({userId,tractoras,semis,esGerente,gastosTodos}) {
  ...
  const calcV=v=>{
    ...
    const consumo=t?calcConsumoHistorico(gastosTodos,t.id)||(parseFloat(t.consumo_estimado)||28):28;
  ...
  const saveViaje=async()=>{
    if(!modal.km||!modal.precio)return;
    const{base,iva}=calcIVA();
    const payload={fecha:modal.fecha,cliente:modal.cliente,origen:modal.origen,destino:modal.destino,pais:modal.pais,km:modal.km,km_vuelta:vuelta?(modal.km_vuelta||modal.km):"",peaje:modal.peaje,precio:modal.precio,tiene_iva:modal.tiene_iva,tipo_iva:modal.tipo_iva,base_imponible:modal.tiene_iva?base:null,iva_amount:modal.tiene_iva?iva:null,truck_id:modal.truck_id,semi_id:modal.semi_id,user_id:userId};
    if(editando?.id){
      ...
      {modal&&<div className="ov" onClick={()=>setModal(false)}>
        <div className="modal" onClick={e=>e.stopPropagation()}>
          ...
          <div className="g2">
            ...
            <div className="fld"><label className="lbl">Precio cobrado (€)</label><input className="inp" type="number" placeholder="0" value={modal.precio} onChange={e=>setModal({...modal,precio:e.target.value})}/></div>
          </div>
...
function ResumenPage({userId,tractoras,semis}) {
  ...
  const exportarExcel=()=>{
    ...
    if(expTipo==="viajes"||expTipo==="todo"){
      csv+="VIAJES\nFecha,Origen,Destino,Cliente,Km,Precio,IVA,Peaje,Tractora\n";
      vFilt.forEach(v=>{const t=tractoras.find(x=>x.id===v.truck_id);csv+=`${v.fecha},"${v.origen||""}","${v.destino||""}","${v.cliente||""}",${v.km},${v.precio},${v.iva_amount||0},${v.peaje||0},"${t?.matricula||""}"\n`;});
    }
    if(expTipo==="gastos"||expTipo==="todo"){
      csv+="\nGASTOS VARIABLES\nFecha,Tipo,Importe,Vehículo,Nota\n";
      gFilt.forEach(g=>{const v=[...tractoras,...semis].find(x=>x.id===g.vehicle_id);csv+=`${g.fecha},"${g.tipo}",${g.importe},"${v?.matricula||""}","${g.nota||""}"\n`;});
    }
    const blob=new Blob(["\uFEFF"+csv],{type:"text/csv;charset=utf-8;"});
    const url=URL.createObjectURL(blob);
    const a=document.createElement("a");a.href=url;a.download=`KmRentable_${expTipo}_${expPeriodo}.csv`;a.click();
    ...
    <div className="page fu">
      ...
      {modalExport&&<div className="ov" onClick={()=>setModalExport(false)}>
        <div className="modal" onClick={e=>e.stopPropagation()}>
          ...
          <div className="fld"><label className="lbl">¿Qué quieres exportar?</label>
            <div style={{display:"flex",flexDirection:"column",gap:"0.5rem",marginTop:"0.25rem"}}>
              {[["todo","Viajes + Gastos (todo)"],["viajes","Solo viajes"],["gastos","Solo gastos"]].map(([v,l])=>(
                <div key={v} style={{display:"flex",alignItems:"center",gap:"0.75rem",padding:"0.75rem",background:expTipo===v?"var(--a1)15":"var(--s2)",border:`1px solid ${expTipo===v?"var(--a1)":"var(--border2)"}`,borderRadius:"var(--r2)",cursor:"pointer"}} onClick={()=>setExpTipo(v)}>
          ...
          <div className="fld"><label className="lbl">¿De qué período?</label>
            <select className="inp sel" value={expPeriodo} onChange={e=>setExpPeriodo(e.target.value)}>
              ...
              <optgroup label="Por año">
                {[nowAno(),String(parseInt(nowAno())-1)].map(a=><option key={a} value={a}>{a} (año completo)</option>)}
              </optgroup>
            </select>
          </div>
          <div className="mact">
...
function InicioPage({userId,tractoras,semis,perfil,esGerente,gastosTodos,viajesTodos,setViajesTodos,gastosFijos}) {
  ...
  if(tractoras.length===0)return(
    <div className="page fu">
      <div><div style={{fontFamily:"'Bebas Neue'",fontSize:"1.4rem",letterSpacing:"0.04em",color:"var(--text)"}}>KmRentable</div><div style={{fontSize:"0.83rem",color:"var(--muted)",marginTop:"0.25rem"}}>{perfil.empresa||perfil.nombre||""}</div></div>
      <div className="card"><div className="chd">Para empezar</div>
        <div style={{display:"flex",flexDirection:"column",gap:"0.75rem"}}>
          {[["①","Añade tu tractora","Ve a Flota y registra tu vehículo"],["②","Configura los gastos fijos","En Gastos → sección gastos fijos"],["③","Registra tu primer viaje","Ruta, km y precio cobrado"]].map(([n,t,s])=>(
  ...
  const totalIngAno=viajesTodos.filter(v=>v.fecha?.startsWith(nowAno())).reduce((s,v)=>s+(parseFloat(v.precio)||0),0);
  const benAno=viajesTodos.filter(v=>v.fecha?.startsWith(nowAno())).reduce((s,v)=>s+calcV(v).ben,0)-(totalFijosMes*new Date().getMonth());

  const costesKm=tractoras.map(t=>({t,coste:calcCosteKmCompleto(t,gastosFijos,gastosTodos,viajesTodos,parseFloat(t.km_mensuales)||0)})).filter(x=>x.coste>0);
  ...
  const clientes=Object.entries(cMap).map(([n,d])=>({n,margen:d.ing>0?((d.ing-d.cost)/d.ing)*100:0})).sort((a,b)=>b.margen-a.margen).slice(0,5);

  return(
    <div className="page fu">
      <div>
        <div style={{fontFamily:"'Bebas Neue'",fontSize:"1.4rem",letterSpacing:"0.04em",color:"var(--text)"}}>KmRentable</div>
        <div style={{fontSize:"0.83rem",color:"var(--muted)",marginTop:"0.25rem"}}>{perfil.empresa||perfil.nombre||""} · {tractoras.length} tractora{tractoras.length!==1?"s":""}</div>
      </div>
      ...
      {esGerente&&costesKm.length>0&&<div className="card">
        <div className="chd">Coste €/km por tractora</div>
        <div style={{display:"flex",flexDirection:"column",gap:"0.5rem"}}>
          {costesKm.map(({t,coste},i)=>(
            <div key={i} style={{display:"flex",justifyContent:"space-between",alignItems:"center",padding:"0.5rem 0",borderBottom:i<costesKm.length-1?"1px solid var(--border)":"none"}}>
              <div>
                <div style={{fontWeight:600,fontSize:"0.85rem"}}>🚛 {t.matricula||"Sin mat."}{t.apodo?` "${t.apodo}"`:"" }</div>
                <div style={{fontSize:"0.68rem",color:"var(--muted)",marginTop:2}}>Fijos + variables + combustible</div>
              </div>
              <div style={{fontFamily:"'Bebas Neue'",fontSize:"1.3rem",color:"var(--a1)",letterSpacing:"0.02em"}}>{eurosKm(coste)}</div>
            </div>
          ))}
        </div>
      </div>}
      ...
      {esGerente&&<>
        ...
        <div className="sgrid">
          <div className="stat"><div className="slbl">Ingresos {nowAno()}</div><div className="sval g">{euros(totalIngAno)}</div></div>
          <div className="stat"><div className="slbl">Beneficio {nowAno()}</div><div className={`sval ${benAno>=0?"g":"r"}`}>{euros(benAno)}</div></div>
        </div>
      </>}
      ...
      <div style={{fontSize:"0.72rem",color:"var(--muted2)",textAlign:"center",paddingTop:"0.5rem"}}>{viajesTodos.length} viajes registrados en total</div>
    </div>
  );
}
...
export default function App() {
  ...
  return(
    ...
    <div className="app">
      ...
      {tab==="inicio"&&<InicioPage userId={user.id} tractoras={tractoras} semis={semis} perfil={perfil} esGerente={esGerente} gastosTodos={gastosTodos} viajesTodos={viajesTodos} setViajesTodos={setViajesTodos} gastosFijos={gastosFijos}/>}
      ...
      {tab==="gastos"&&<GastosPage userId={user.id} tractoras={tractoras} semis={semis} esGerente={esGerente} accentIdx={perfil.accent_idx||0}/>}
