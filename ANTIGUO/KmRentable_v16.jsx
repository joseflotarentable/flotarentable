...
function GastosPage({userId,tractoras,semis,esGerente,accentIdx}) {
  ...
  const saveGasto=async()=>{
    if(!modal.importe||parseFloat(modal.importe)<=0){setToast("⚠️ Introduce un importe válido");return;}
    const payload={fecha:modal.fecha,tipo:modal.tipo,importe:parseFloat(modal.importe),litros:modal.litros?parseFloat(modal.litros):null,precio_litro:modal.precio_litro?parseFloat(modal.precio_litro):null,odometro:modal.odometro?parseFloat(modal.odometro):null,pais:modal.pais||"España",vehicle_id:modal.vehicle_id||null,vehicle_tipo:modal.vehicle_tipo||"tractora",nota:modal.nota||"",mes:mesFiltro,ano:anoActual,user_id:String(userId)};
    if(editGasto?.id){
...
export default function App() {
  ...
  const tabs=[{id:"inicio",lbl:"Inicio",icon:I.dash},{id:"flota",lbl:"Flota",icon:I.truck},{id:"viajes",lbl:"Viajes",icon:I.trend},{id:"gastos",lbl:"Gastos",icon:I.coin},{id:"analizar",lbl:"Analizar",icon:I.analyze}];
