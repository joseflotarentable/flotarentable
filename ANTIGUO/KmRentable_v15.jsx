...
function GastosPage({userId,tractoras,semis,esGerente,accentIdx}) {
  ...
  const saveGasto=async()=>{
    ...
    const payload={fecha:modal.fecha,tipo:modal.tipo,importe:modal.importe,litros:modal.litros||null,precio_litro:modal.precio_litro||null,odometro:modal.odometro||null,pais:modal.pais||"España",vehicle_id:modal.vehicle_id,vehicle_tipo:modal.vehicle_tipo||"tractora",nota:modal.nota||"",mes:mesFiltro,ano:anoActual,user_id:userId};
    if(editGasto?.id){
                ...
                {v.conceptos.map(concepto=>{
                  ...
                  return(
                    <div key={concepto} className="gfijo-row">
                      ...
                      <input className="gfijo-inp" type="number" placeholder="0" defaultValue={fijo?.importe||""} key={fijo?.id||concepto} onBlur={e=>{if(e.target.value!==String(fijo?.importe||""))saveFijo(v.id,concepto,e.target.value,esAnual?"anual":"mensual");}}/>
