...
async function geocode(q) {
  ...
  try {
    const r = await fetch(`https://nominatim.openstreetmap.org/search?q=${encodeURIComponent(q)}&format=json&limit=8&addressdetails=1&featuretype=city`,{headers:{"Accept-Language":"es","User-Agent":"KmRentable/1.0"}});
...
.modal{background:var(--s1);border:1px solid var(--border2);border-radius:22px 22px 0 0;width:100%;max-width:430px;max-height:78vh;overflow-y:auto;padding:1.5rem;padding-bottom:2rem;display:flex;flex-direction:column;gap:0.875rem}
  ...
  const handleRegister=async()=>{
    ...
    if(step===2){
      ...
      if(form.rol==="chofer"){
        ...
        if(!emp){setErr("Codigo incorrecto. Pidele el codigo KM-XXXX a tu gerente.");return;}
      }
        ...
        {step===2&&<><div style={{fontFamily:"'Bebas Neue'",fontSize:"1.8rem",letterSpacing:"0.04em"}}>Tu rol</div>
          ...
          {form.rol==="chofer"&&<>
            ...
            <div className="fld">
              ...
              <input className="inp" placeholder="KM-XXXX" value={form.codigoEmpresa} onChange={e=>setForm({...form,codigoEmpresa:e.target.value.toUpperCase()})} style={{letterSpacing:"0.1em",fontWeight:700,fontSize:"1rem"}}/>
            </div>
          </>}</>}
      ...
      {modal&&<div className="ov" onClick={()=>setModal(false)}>
        <div className="modal" onClick={e=>e.stopPropagation()}>
          ...
          {(()=>{
            ...
            const consumo=parseFloat(t?.consumo_estimado)||28;
            const precioG=t?calcPrecioMedioGasoil(gastosTodos,t.id)||1.8:1.8;
...
function ResumenPage({userId,tractoras,semis}) {
  ...
  const exportarExcel=()=>{
    ...
    const a=document.createElement("a");a.href=url;a.download=`KmRentable_${expTipo}_${periodo}.csv`;a.click();
...
function InicioPage({userId,tractoras,semis,perfil,esGerente,gastosTodos,viajesTodos,setViajesTodos,gastosFijos}) {
  ...
  const costesKm=tractoras.map(t=>({t,coste:calcCosteKmCompleto(t,gastosFijos,gastosTodos,viajesTodos)})).filter(x=>x.coste>0);
  ...
  return(
    <div className="page fu">
      ...
      {esGerente&&costeEmpresa>0&&<div className="hcard">
        <div className="hlbl">Coste €/km general empresa</div>
        <div className="hval">{eurosKm(costeEmpresa)}</div>
        <div className="hsub">Fijos + variables + combustible · todos los vehículos · {mesLabel}</div>
      </div>}

      {esGerente&&costesKm.length>0&&<div className="card">
        <div className="chd">Coste €/km por tractora — {mesLabel}</div>
        <div style={{display:"flex",flexDirection:"column",gap:"0.5rem"}}>
          {costesKm.map(({t,coste},i)=>(
            <div key={i} style={{display:"flex",justifyContent:"space-between",alignItems:"center",padding:"0.5rem 0",borderBottom:i<costesKm.length-1?"1px solid var(--border)":"none"}}>
              <div>
                ...
                <div style={{fontSize:"0.68rem",color:"var(--muted)",marginTop:2}}>Fijos proporcionales + variables + combustible</div>
              </div>
...
function AnalizarPage({userId,tractoras,semis,gastosTodos,viajesTodos,gastosFijos}) {
  ...
  const simResult=()=>{
    ...
    const consumo=parseFloat(t?.consumo_estimado)||28;
    const precioG=t?calcPrecioMedioGasoil(gastosTodos,t.id)||precioGasoilGlobal():precioGasoilGlobal();
    const costeGasoil=km*(consumo/100)*precioG;
  ...
  return(
    <div className="page fu">
      ...
      <div className="tab-row">
        {[["sim","Simulador"],["clientes","Clientes"],["tendencia","Tendencia"]].map(([id,lbl])=>(
...
export default function App() {
  ...
  const tabs=[{id:"inicio",lbl:"Inicio",icon:I.dash},{id:"flota",lbl:"Flota",icon:I.truck},{id:"gastos",lbl:"Gastos",icon:I.coin},{id:"viajes",lbl:"Viajes",icon:I.trend},{id:"analizar",lbl:"Analizar",icon:I.analyze}];
  ...
  return(
    ...
    <div className="app">
      ...
      {tab==="analizar"&&esGerente&&<AnalizarPage userId={user.id} tractoras={tractoras} semis={semis} gastosTodos={gastosTodos} viajesTodos={viajesTodos} gastosFijos={gastosFijos}/>}
