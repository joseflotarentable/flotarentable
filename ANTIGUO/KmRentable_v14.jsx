...
function RegistrarPage({userId,tractoras,semis,esGerente,gastosTodos,accentIdx}) {
  ...
  return(
    <div className="page fu">
      ...
      {subtab==="viajes"&&<ViajesPage key="viajes" userId={userId} tractoras={tractoras} semis={semis} esGerente={esGerente} gastosTodos={gastosTodos}/>}
      {subtab==="gastos"&&<GastosPage key="gastos" userId={userId} tractoras={tractoras} semis={semis} esGerente={esGerente} accentIdx={accentIdx}/>}
    </div>
  );
}
  ...
  return(
    <div className="page fu">
      ...
      <div className="tab-row" style={{display:"grid",gridTemplateColumns:"repeat(4,1fr)"}}>
        {[["sim","Simular"],["clientes","Clientes"],["tendencia","Tendencia"],["resumen","Resumen"]].map(([id,lbl])=>(
    ...
    <div className="app">
      <div className="hdr">
        <div className="hdr-left">
          ...
          <div><div className="hdr-brand">{perfil.empresa||"KmRentable"}</div><div className="hdr-sub">{esGerente?"Gerente":"Chofer"} · {tractoras.length} tractora{tractoras.length!==1?"s":""}</div></div>
        </div>
        <div style={{display:"flex",alignItems:"center",gap:"0.5rem"}}>
